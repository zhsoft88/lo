# LO (Little Octopus, 小章鱼) — 先进版本控制系统

LO (Little Octopus, 小章鱼) 是一个轻量级、内容寻址的版本控制系统，使用 Go 语言编写。它采用 SHA256 内容哈希作为对象标识，支持智能大文件分块（CDC）、跨平台文件变体、HTTP/SSH 远程同步、子模块等特性。

## 解决的痛点

- 跨平台文件管理的问题
  - lo: 不同平台可有平台相关的文件，克隆时只会将全平台适用和本平台的文件克隆下来，其它平台的文件仅可通过命令查看
...
- **`.loignore`** — 类似 `.gitignore` 的忽略规则

## 安装

```bash
go install github.com/zhsoft88/lo/cmd/lo@latest
```

或从源码构建：

```bash
git clone https://github.com/zhsoft88/lo
cd lo
mkdir -p dist && go build -o dist/lo ./cmd/lo
```
...
# 远程协作
lo remote add origin http://example.com/repo
lo push origin
lo pull origin
lo clone http://example.com/repo myrepo
```
...
lo 内建跨平台文件变体支持，可为不同操作系统维护同一文件的独立版本：

```bash
# 添加当前 OS 定制的文件变体
lo add --os config.ini

# 查看文件的所有变体
lo show config.ini

# 查看特定变体的内容
lo show config.ini --os win
lo show config.ini --os linux
```
...
对象存储在 `.lo/objects/XX/YYYYYY` 路径（类似 Git 的布局），内容格式为：
...
- 可通过 `lo config` 调整分块参数：

```bash
lo config core.chunk_size_min     # 查看最小值
lo config core.chunk_size 2097152  # 调整为 2MB 平均分块
```

### LFS 懒加载

当克隆大文件仓库时，`--lazy` 选项会跳过实际分块数据的传输，文件在磁盘上以 `lo-lfs` 占位符标记。之后可按需拉取：

```bash
# 懒克隆
lo clone --lazy http://example.com/large-repo myrepo
